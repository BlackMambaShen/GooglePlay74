...
public class MyFlowLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i <childCount ; i++) {
            ...
            int childHeightMeasureSpec=MeasureSpec.makeMeasureSpec(height,(widthMode==MeasureSpec.EXACTLY)?MeasureSpec.AT_MOST:heightMode);
            //开始测量
            ...
            if (mUsedWidth<width){//判断是否超出了边界
                ...
                //已超出边界
                //1.当前没有任何元素，一添加当前子控件，就超出边界（子控件很长0
        ...
        totalHeight+=getPaddingBottom()+getPaddingTop();//增加上下边距
...
//        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
    ...
    class Line{
        ...
        public int mMaxHeight;//当前控件的高度（以最高控件为准）
        private ArrayList<View>mChildViewList=new ArrayList<View>();//当前所有子控件集合
        //添加一个子控件
        ...
        public void layout(int left,int top){
            ...
            //剩余空间分配给每个子控件
            int validWidth = getMeasuredWidth() - getPaddingLeft() - getPaddingRight();//屏幕总宽度
            //计算剩余宽度
            int suplusWidth = validWidth - mTotalWidth-(getChildCount()-1)*mHorizontalSpacing;
            if (suplusWidth>=0){
                //有剩余空间
                int space = suplusWidth / childCount;//平均每个控件分配的大小
                //重新测量子控件
                for (int i = 0; i <childCount ; i++) {
                    ...
                    int widthMakeMeasureSpec = MeasureSpec.makeMeasureSpec(measuredWidth, MeasureSpec.EXACTLY);
                    int heightMakeMeasureSpec = MeasureSpec.makeMeasureSpec(measuredHeight, MeasureSpec.AT_MOST);
                    //重新测量控件
                    childView.measure(widthMakeMeasureSpec,heightMakeMeasureSpec);
